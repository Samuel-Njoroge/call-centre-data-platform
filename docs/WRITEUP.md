...
than routing them through Airbyte.** This repo uses Airbyte (S3 → Redshift) because the case study
handed us pre-exported CSVs and asked us to load files repeatably — that's not what real production
ingestion for these two systems should look like. Neither is likely to have a maintained Airbyte
connector (a niche telephony platform; d.light's own CRM), so the honest production path is a custom
